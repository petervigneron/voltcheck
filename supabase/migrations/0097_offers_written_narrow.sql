-- listing_offers is written like listing_seen: a narrow touch per sighting.
--
-- 2026-09-23, 16:19 UTC. The nightly's Sync to Supabase step failed on a
-- statement timeout — its first run through 0094. That migration wrote the
-- full offers row, jsonb payload included, for every sighting in a chunk,
-- on a table carrying two extra btree indexes on last_seen_at; the old
-- function wrote one narrow listing_seen row per sighting. On an instance
-- whose IO is the budget (0042), a 3 MB chunk of a few thousand rows paid
-- for thousands of TOASTed payload rewrites and index updates it did not
-- need, on top of the listings upsert that already sat near the limit.
--
-- Now: a (vin, site) pair is written in full only when it is new or its
-- price changed; every other sighting updates last_seen_at and last_run,
-- and with the two extra indexes dropped that is a heap-only update. The
-- primary key (vin, dealer_domain) serves every read: the incumbent probe
-- by (vin, site), the move candidates and the view's rule 2 by vin prefix.
-- Measured after apply, 3,000 unchanged live rows through the RPC.

drop index if exists listing_offers_vin_seen_idx;
drop index if exists listing_offers_seen_idx;
