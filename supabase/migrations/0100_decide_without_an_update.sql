-- The headline decision is computed in one statement, not by an UPDATE.
--
-- 2026-09-23, 19:58 UTC. The recovery sync failed in eight seconds:
--   ERROR: UPDATE requires a WHERE clause
--   SQL statement "update _decide d set takes_headline = ..."
-- The database runs pg-safeupdate for the API role, so an UPDATE with no
-- WHERE is refused when ingest_listings runs through PostgREST — the path
-- db-sync and every lane use — and allowed on the admin path the control
-- test used. The 0094 control test was therefore run through the wrong
-- door; the memory note that said "verify through the real path" was
-- written for exactly this and not followed.
--
-- takes_headline is now a column of the same CREATE TABLE AS that builds
-- _decide. Nothing else changes. Verified through functions/v1/ingest with
-- the ingest token, the way db-sync calls it, before the recovery re-run.
