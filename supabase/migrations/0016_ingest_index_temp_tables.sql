-- ingest_listings scaled O(n²) and started timing out. When the OEM-locator
-- lanes (2026-08-15) pushed the feed from ~15k to ~39k rows — including huge
-- *complete* domains (cadillac.com ~11.6k, chevrolet.com ~10.9k) — db-sync's
-- per-chunk ingest hit the edge function's ~150s wall clock and returned 503,
-- blocking every sync.
--
-- Cause: _incoming and _prior are temp tables with no index on vin and no
-- ANALYZE. The delisting step's `not exists (select 1 from _incoming i where
-- i.vin = l.vin)` — and every other join/anti-join on vin — had no stats, so
-- the planner chose nested loops (a sequential scan of _incoming per candidate
-- row). For a complete domain that is O(existing × incoming): ~11.6k² for
-- Cadillac alone.
--
-- Fix: index and ANALYZE both temp tables right after they are built, so the
-- anti-join and joins go hash-based (O(n)). Behaviour is byte-for-byte
-- identical to 0013 — this is a pure performance change. Reversible by
-- re-applying 0013.
...
begin
  ...
  -- join and the delisting anti-join hash instead of nested-loop.
