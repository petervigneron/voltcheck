-- Forty-eight hours, not thirty-six.
--
-- 2026-09-23, owner: "I don't even really understand the 36 hour rule. I
-- didn't make it. My only objective here is to make the site display
-- accurate and timely information about listings." He is right that the
-- number was a session's choice (0091), not his; what he asked for on
-- 09-12 was that a sold car never be shown as for sale again.
--
-- Measured tonight against a 36-hour window: 172,452 cars for sale in the
-- table, 168,412 shown; 1,069 more would show under 48 hours, and 3 of
-- those carry any evidence of being gone (a recheck miss). The
-- hours-since-seen histogram has clumps at 26, 36, 50 and 75 hours — whole
-- lanes that run every one, two or three days — so a car in a two-day lane
-- spends part of every cycle hidden by the clock alone. That is the
-- daily count swing the owner saw ("well below 600 for most of the day"
-- on the Lightning hub), and today's failed syncs stretched it to six
-- hours. Widening to 48 covers one missed daily cycle plus jitter.
--
-- The same 48 hours is used for rule 2 (a marketplace car on its own
-- page's word or a dealer-site sighting) and for ingest_listings'
-- incumbent-liveness test, so a car the site shows on a 40-hour-old
-- sighting cannot lose its headline to a sibling site in the meantime.
-- Rule 3 is unchanged in substance: a returning car still needs its own
-- page's word. Dry run before apply: 168,412 → 169,481 rows, none dropped.
--
-- This is the interim setting. The rule that replaces the clock — shown
-- while a car was in the latest crawl of its own site, withheld once that
-- site was crawled without it or its page said gone — is written up in
-- docs/agents/freshness-rule-design-2026-09-23.md and needs one new fact
-- (each site's last complete crawl) the ingest does not yet record.
...
begin
  ...
  -- Two writes, sized to what changed. A (vin, site) pair seen for the first
  -- time, or at a new price, gets its full row — price, page, dealer name,
  -- payload. Every other sighting is a narrow touch of last_seen_at, which
  -- no index covers, so it is a heap-only update: the same cost shape as
  -- listing_seen. 0094 rewrote the whole row, payload included, on every
  -- sighting, and the nightly's first chunk of the evening went past the
  -- 60 s statement timeout on 2026-09-23.
  ...
  set last_seen_at = now(), last_run = _run_id
  from _incoming i
  left join _prior p using (vin)
