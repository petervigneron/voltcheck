...
export function PriceSparkline({ history }: { history: { priceUsd: number; observedAt: string }[] }) {
  ...
  const vals = pts.map((p) => p.priceUsd);
  ...
  let d = `M ${M.l} ${py(first.priceUsd).toFixed(1)}`;
  ...
  const wash = `${d} L ${W - M.r} ${yNow.toFixed(1)} L ${M.l} ${yNow.toFixed(1)} Z`;

  const steps = pts.slice(1).map((p, i) => ({
    delta: p.priceUsd - pts[i].priceUsd,
    at: p.observedAt,
  }));
  const title = pts.map((p) => `${monthDay(p.observedAt)}: ${usd(p.priceUsd)}`).join(" → ");
  ...
  return (
    <figure className="mt-4">
      ...
      >
        ...
        <circle cx={px(t1)} cy={yNow} r="4.5" fill={COBALT} stroke={PAPER} strokeWidth="2" />
        ...
        {/* Both ends carry their dollar value. Placed above their own runs and
            ...
            price puts the start label inside the wash. */}
        <text
          x={M.l}
        ...
        {/* The last change always lands at the same x (the tail is fixed), so
            ...
            the left. */}
        <text x={px(t1)} y={H - M.b + 15} textAnchor="middle" fontSize="10" fill={INK} opacity="0.5" className="tabular-nums">
          {monthDay(last.observedAt)}
        </text>
      </svg>

      {/* The story, at a size a phone can read. Two or three points is not a
          trend, it is one or two sentences — so print the sentences. */}
      <div className="mt-1.5 space-y-0.5 text-[12px] leading-snug tabular-nums">
        {steps.length <= 3 ? (
          ...
          <p className="text-ink/55 dark:text-zinc-400">
            <span className="font-semibold text-ink dark:text-zinc-100">{steps.length} price changes</span> since{" "}
            {monthDay(first.observedAt)}, {usd(first.priceUsd)} to {usd(last.priceUsd)}
          </p>
        )}
      </div>
    </figure>
