...
// The saved grid: localStorage names the cars, the same card index the browse
// grid downloads says which of them are still for sale. A saved car the index
// no longer carries has been delisted — it renders as exactly that, with the
// title and price snapshotted at save time, rather than vanishing (a shopper
// wondering where their car went) or rendering stale data as if it were live.
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-0 sm:px-6 sm:py-6">
      <div className="border-t-[3px] border-l-[3px] border-ink">
        <div className={`${CELL} bg-paper px-5 py-4`}>
          <span className="text-[10.5px] font-extrabold tracking-[0.14em] text-ink/60 uppercase tabular-nums">
            Saved cars{entries !== null ? ` · ${entries.length}` : ""}
          </span>
        </div>
      </div>
      ...
      {entries !== null && entries.length === 0 ? (
        ...
        // Without the index we can't tell "still for sale" from "delisted",
        // so the page claims neither.
        <div className="border-l-[3px] border-ink">
