...
A shard body is \`{ v: 1, t, h, pn, r }\`. \`t\` (fact chips), \`h\` (image origins) and \`pn\` (incentive program names) each hold one copy of a repeated thing; \`r\` is the cars, pointing into them by index. A car's keys:

\`i\` id, the VIN in lower case · \`q\` a lower-case string of the words the car can be searched by · \`y\` year · \`k\` make · \`o\` model · \`n\` title · \`p\` asking price in dollars · \`f\` present and 0 when that price is not one we could confirm · \`m\` odometer miles · \`cd\` condition, 0 new 1 used 2 certified · \`d\` drive, 0 RWD 1 AWD 2 FWD · \`b\` body, 0 suv 1 sedan 2 truck 3 van 4 hatchback · \`ct\` city · \`st\` state · \`l\` [latitude, longitude] · \`g\` image, [index into \`h\`, the rest of the URL] · \`tr\` trim · \`kw\` battery capacity in kWh · \`rm\` range in miles · \`hp\` heat pump, 0 yes 1 no 2 verify · \`pr\` 1 when this car's battery has been replaced · \`bb\` 1 when the seller's own description discloses a manufacturer repurchase · \`lo\` the date the car appeared on the seller's site · \`c\` the latest price cut, \`{a: amount, t: date, pv: the price before}\` · \`am\` this price against the same version listed right now, [difference in dollars, how many cars that is, 1 when they are the same trim] · \`as\` a comparison against recorded sales that no page on this site shows, because we do not stand behind it outside the Northwest · \`ic\` incentive, [index into \`pn\`, the program's figure, the cap when the price sits over it, how many programs the car meets, 1 when the lead program is a utility's, state] · \`ts\` indices into \`t\`.
