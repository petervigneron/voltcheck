...
// report listings, the plug-ins are live Voltcheck listings.
...
const vpic = (vin: string, trim: string, modelYear = 2018): VinDecode => ({
  vin,
  usMarket: true,
  make: "KIA",
  model: "Niro",
...
test("a real 2018 Niro hybrid decoding the shared grade-C trim matches nothing", () => {
  // vPIC reads each of these as "Strong HEV (Hybrid Electric Vehicle)".
  for (const vin of ["KNDCC3LC1J5160937", "KNDCC3LC2J5112038", "KNDCC3LC3J5128653", "KNDCC3LCXJ5133428", "KNDCC3LC2J5146237"]) {
    const r = matchEnrichment(vpic(vin, MIXED), null);
...
test("the plug-in wearing the same trim string still resolves, on position 8 alone", () => {
  ...
  // hybrids above.
  assert.equal(matchEnrichment(vpic("KNDCC3LD4J5144563", MIXED), null).exact?.id, "niro-phev-2018-22-alt");
  assert.equal(matchEnrichment(vpic("KNDCM3LD2J5140038", "LX (PHEV)"), null).exact?.id, "niro-phev-2018-22-alt");
  assert.equal(matchEnrichment(vpic("KNDCD3LD2K5281581", "EX (PHEV)", 2019), null).exact?.id, "niro-phev-2018-22-alt");
});
