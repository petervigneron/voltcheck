# Nissan Leaf (model years 2019 and 2024, and the all-new 2026): Charging

## Not every Leaf can fast charge at all

- Nissan sold DC fast charging as an option. Nissan's own words, unchanged from the 2019 manual to the 2024 one: "Quick charge capability is only available on vehicles manufactured with the quick charge option, which includes the quick charge port. If your vehicle does not have such a port, quick charging cannot be used."[^1][^2]
- The 2019 manual titles the section "Quick charge (if so equipped)."[^1]
- This is a per-car check, not a per-trim one. Nothing in the model name says which Leaf has the port.

## The connector

- A quick-charge-equipped Leaf uses CHAdeMO, not CCS and not NACS. Nissan: "A vehicle equipped with a quick charge port is compatible with most CHAdeMO (Japanese industry standard) connectors on charging stations."[^1][^2]
- Nissan states the risk plainly in its own manual: "While supported by NISSAN, this connector may not become the US SAE standard."[^1][^2]
- For AC charging: "This vehicle is compatible with any public charging station that is SAE J1772 compliant."[^1][^2]
- The all-new 2026 Leaf changes both ports. Nissan: "A familiar J1772 charge port on the driver's-side fender allows for plugging in at home and many Level 2 public chargers, while the NACS fast charge port on the passenger-side fender unlocks the faster speeds of over 20,000 Tesla Superchargers."[^3]
...
## What Nissan tells Leaf owners about repeat fast charging

These are Nissan's own instructions in the owner's manual, not third-party advice.
...
- Nissan's 2024 manual describes the home charger by its current, not its power: "Normal charge uses the charging device (AC 220 - 240 volt, 16A or 32A) that can be installed in your home to charge the Li-ion battery."[^2]
- Trickle charging uses "an AC 110–120 volt, 15A dedicated outlet," and Nissan states it "is not recommended for regular use because it takes more time than normal charging."[^2]
- Onboard charger power in kW: not covered. The 2019 and 2024 owner's manuals state no kW figure for the onboard AC charger, so none is claimed here.
...
[^1]: Nissan, *2019 LEAF Owner's Manual* (PDF), "Charging," printed pages CH-6 through CH-9: the quick-charge and connector text on CH-6–CH-7, the 40 kWh charge-time table on CH-8, and the 62 kWh charge-time table on CH-9. Nissan prints no figure ID beside the 2019 table; the label LHA5087 belongs to the 2024 manual's version of the same table, and is cited there instead. Nissan's own host: https://www.nissanusa.com/content/dam/Nissan/us/manuals-and-guides/leaf/2019/2019-Nissan-LEAF-owner-manual.pdf. Both charge-time tables are images, not text; they were transcribed from the pages rendered at 400 dpi, not from extracted text.
...
[^4]: Nissan, *2024 LEAF Owner's Manual* (PDF), "EV Overview," printed pages EV-22 and EV-23. Same file as above.

[^5]: Nissan, *2024 LEAF Owner's Manual* (PDF), "Li-ion battery warmer (if so equipped) — For models with 40 kWh battery," printed page EV-5. Same file as above.

[^6]: Nissan, *2024 LEAF Owner's Manual* (PDF), "Li-ion battery warmer (if so equipped) — For models with 60 kWh battery," printed page EV-6. Same file as above.

## Scope note

Two model years of the second-generation Leaf were opened directly: 2019 and 2024, both Nissan's own owner's manuals from Nissan's own host. Model years 2011 through 2018, 2020 through 2023, and 2025 were not checked and are not claimed — although the CHAdeMO and quick-charge-option wording is identical in the two years that were, five model years apart.

The 2026 Leaf is a different, third-generation car. Every 2026 fact here comes from Nissan's own June 17, 2025 press release and is labeled as 2026 in the sentence that states it. Nothing from that release is applied to an earlier Leaf.
